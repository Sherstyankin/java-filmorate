...
public class UserService {
    ...
    public void addFriend(Long userId, Long friendId) {
        User user1 = userStorage.getUsers().get(userId); //достаем юзера из мапы по userId
        ...
        User user2 = userStorage.getUsers().get(friendId); //достаем друга из мапы по friendId
    ...
    public void deleteFriend(Long userId, Long friendId) {
        ...
        User user2 = userStorage.getUsers().get(friendId); //достаем юзера из мапы по userId
    ...
    public List<User> getUserFriends(Long userId) {
        ...
        for (Long id : friendsId) {
            userFriends.add(userStorage.getUsers().get(id));//достаем друзей
        }
    ...
    public List<User> getCommonFriends(Long userId, Long otherId) {
        User user1 = userStorage.getUsers().get(userId); //достаем user1 из мапы по userId
        User user2 = userStorage.getUsers().get(otherId); //достаем user2 из мапы по otherId
        ...
        for (Long id : commonFriendsId) {
            commonFriends.add(userStorage.getUsers().get(id));//достаем общих друзей
        }
